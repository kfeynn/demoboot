...
<form action="/malladmin/User/suplist" method="post">
...
<div class="dataList">
    <table   style="width:100%" >
        ...
        <tr>
            ...
            <td>${row.ID}</td>
            <td>${row.ID}</td>
            <td><a class="editOperate" href="${pageContext.request.contextPath}/malladmin/User/edit?sid=${row.ID}">[编辑]</a></td>
            <td><a class="editOperate" href="${pageContext.request.contextPath}/malladmin/User/delete?sid=${row.ID}" onclick = "if(!confirm('您确认要删除该内容吗?')) return false;">[删除]</a></td>
            <td><a class="editOperate" href="${pageContext.request.contextPath}/malladmin/User/userAuthorization?sid=${row.ID}">[分配权限]</a></td>
        </tr>
        </c:forEach>
