...
@Configuration
@MapperScan(basePackages = { "cn.grand.demoboot.mapper.supplier" },sqlSessionFactoryRef="db2SqlSessionFactoryBean" ) // 扫描的mybatis接口类的包名
public class MybatisDB2Config
{

    @Autowired
    // 必须指定注入哪个数据源，否则找到多个会注入失败
    @Qualifier(DataSources.Supplier)
    private DataSource supplier;

    @Bean(name = "db2SqlSessionFactoryBean")

    @ConfigurationProperties(prefix = "spring.datasource.supplier") // 和 配置文件中的前缀保持一致
    public SqlSessionFactoryBean db2SqlSessionFactoryBean() throws Exception
    {
        SqlSessionFactoryBean sqlSessionFactoryBean = new SqlSessionFactoryBean();
        // 配置数据源
        sqlSessionFactoryBean.setDataSource(supplier);
        // 解决配置到配置文件中通过*配置找不到mapper文件的问题。 如果不设置这一行，在配置文件中，只能使用数组的方式一个个的罗列出来，并且要指定具体的文件名
        sqlSessionFactoryBean.setMapperLocations(new PathMatchingResourcePatternResolver().getResources("classpath:cn/grand/demoboot/mapper/supplier/*.xml"));
